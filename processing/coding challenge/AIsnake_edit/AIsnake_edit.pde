...
float[] worldindex=new float[1000];
...
void draw(){
  ...
  for(int i=0;i<s.length;i+=1){
    ...
    if(s[i].score>generationrecord) {
      generationrecord=s[i].score;
      generationindex=i;
    }
  }
  ...
  information();
  if(deaths==true){
    clonesnake=new snake();
    clonesnake.clone=true;
    generationrecords[(int)generationtime]= generationrecord;
    worldrecords[(int)generationtime]=worldrecord;
    worldindex[(int)generationtime]=generationindex;
    clonesnake.n=s[(int)worldindex[(int)generationtime]].n;
    //clonesnake.location=s[(int)worldindex[(int)generationtime]].rl;
    clonesnake.f=s[(int)worldindex[(int)generationtime]].f;
    //PVector l=clonesnake.location.get();
    //clonesnake.sn=new ArrayList<PVector>();
   // clonesnake.sn.add(l.add(new PVector(-1,0)));
    //l=clonesnake.location.get();
    //clonesnake.sn.add(l.add(new PVector(-1,0)));
    //clonesnake.reset();
    
    //print(clonesnake.location.x,clonesnake.location.y+"\n");
    //print(s[(int)worldindex[(int)generationtime]].rl.x,s[(int)worldindex[(int)generationtime]].rl.y+"\n");
    generate();
    deathn=1000;
    generationtime+=1;
    maxscores=0;
